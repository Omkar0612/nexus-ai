// Package host provides the NEXUS Plugin sandbox host for subprocess plugins.
...
// Registry manages all loaded subprocess plugins.
...
func NewRegistry(pluginDir string) (*Registry, error) {
	...
	if err := os.MkdirAll(pluginDir, 0o750); err != nil {
		return nil, fmt.Errorf("plugin/host: mkdir %s: %w", pluginDir, err)
	}
...
func (r *Registry) Discover() ([]Skill, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("plugin/host: read dir: %w", err)
	}
	var found []Skill
...
func (r *Registry) Invoke(ctx context.Context, name, inputJSON string) (*Result, error) {
	...
	r.mu.RUnlock()
	if !ok {
		return nil, fmt.Errorf("plugin/host: %q not found", name)
	}
	...
	if !strings.HasPrefix(clean+string(filepath.Separator), r.dir+string(filepath.Separator)) {
		return nil, fmt.Errorf("plugin/host: %q path escapes plugin root", name)
	}
	// Cap input size before passing to subprocess.
	if len(inputJSON) > maxInputBytes {
		return nil, fmt.Errorf("plugin/host: input exceeds %d byte limit", maxInputBytes)
	}
	start := time.Now()
	var cmd *exec.Cmd
	switch p.lang {
	...
	default:
		return nil, fmt.Errorf("plugin/host: unknown language %s", p.lang)
	}
	...
	if err != nil {
		...
		return nil, fmt.Errorf("plugin/host[%s]: exec: %w", name, err)
	}
...
func readManifest(dir string) (Skill, Language, error) {
	...
	if m.Name == "" {
		return Skill{}, "", fmt.Errorf("plugin/host: manifest missing name in %s", dir)
	}
